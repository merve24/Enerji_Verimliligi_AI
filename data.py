...
def prepare_rag_data(api_key):
    ...
    try:
        progress_bar = st.progress(0, text="Metin parçaları vektörleştiriliyor...")
        for i, chunk in enumerate(text_chunks):
            response = client.models.embed_content(
                model='text-embedding-004',
                contents=chunk, # Burada tekil metin gönderiliyor
            )
            # Doğru Erişim Yöntemi: API yanıtından 'embedding' anahtarını çekmek
            embeddings_list.append(response['embedding'])
            progress_bar.progress((i + 1) / len(text_chunks), text=f"Metin parçası {i+1}/{len(text_chunks)} işlendi.")
        progress_bar.empty()
    ...
    try:
        ...
        np.save(chunks_path, np.array(text_chunks, dtype=object)) # np.save için array'e çevirmek daha güvenli
