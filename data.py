...
# --- 2. ANA FONKSİYON: VERİSİNİ HAZIRLAMA (API Uyumlu Tekli Gömme) ---
# Bu önbellek (cache), embedding'in yalnızca bir kez hesaplanmasını sağlar.
@st.cache_resource
def prepare_rag_data(api_key):
    ...
    # --- KRİTİK DÜZELTME: Tek tek Gömme (embed_content) Kullanımı ---
    ...
    try:
        ...
        # Her bir metin parçasını tek tek gömme (embed_content)
...
def simple_query_streamlit(prompt, text_chunks, embeddings, api_key):
    ...
    # 2. ÜRETİM AŞAMASI: Gemini API çağrısı (Halüsinasyon Engelleme Talimatı)
    try:
        client = genai.Client(api_key=api_key)
