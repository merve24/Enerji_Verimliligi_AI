...
# Uyumsuz eski importlar kaldırıldı. Hata yakalamak için sadece yeni genai kullanılıyor.
...
def prepare_rag_data(api_key):
    ...
    try:
        with st.status("Metin parçaları vektörleştiriliyor...", expanded=True) as status:
            for i, chunk in enumerate(text_chunks):
                ...
                response = client.models.embed_content(
                    ...
                    # Güvenli erişim için içeriği liste olarak gönderiyoruz
                    contents=[chunk], 
                )
                # KESİN DÜZELTME: response.embeddings[0] ile vektöre doğru erişim
        ...
        # Genel hata yakalama, hata ismini göstermeden önce
