...
def simple_query_streamlit(prompt, text_chunks, embeddings, api_key):
    ...
    # --- 5️⃣ MODEL CEVAP ÜRETİMİ (GÜNCELLENMİŞ) ---
    try:
        client = genai.Client(api_key=api_key)

        # 🟢 GÜNCELLENMİŞ RAG PROMPT 🟢
        rag_prompt = (
        ...
        # 🎯 YENİ: Yapılandırma Bloğu (Timeout ve Temperature içerir)
        config = genai.types.GenerateContentConfig(
            # Akıl yürütmeyi teşvik etmek için sıcaklık 0.4'e ayarlandı.
            temperature=0.4,
            # Cevap vermesi için maksimum 60 saniye bekle. Sonra hata ver.
            timeout=60.0
        )
        ...
        response = client.models.generate_content(
            ...
            contents=rag_prompt,
            config=config # YENİ: Yapılandırmayı ekle
        )

        return response.text, retrieved_text

    except genai.errors.ResourceExhausted as e:
        # Kota aşımı gibi özel API hatalarını yakala.
        st.error("API Kotası Aşıldı: Lütfen daha sonra tekrar deneyin veya API anahtarınızı kontrol edin.")
        return "Üzgünüm, şu anda API çağrı limitleri nedeniyle cevap veremiyorum.", retrieved_text
    except Exception as e:
        # Genel hataları (Zaman Aşımı dahil) yakala ve kullanıcıya bildir.
        st.error(f"Sorgulama Hatası: Gemini API çağrısında bir sorun oluştu. Hata: {e}")
        return "Üzgünüm, sorgulama sırasında bir hata oluştu. Lütfen logları kontrol edin.", retrieved_text