...
# Streamlit'in @st.cache_resource ile uyumlu, güvenli dönüş değerleri içerir.
def prepare_rag_data(api_key):
    # Boş bir indeks oluşturma fonksiyonu (Hata durumunda güvenli dönüş için kullanılır)
    def create_empty_rag():
        # FAISS 1536 boyutunda boş bir indeks oluşturur (Gemini Embedding boyutu)
    ...
    try:
        ...
        # Hata döngüsünü engellemek için uyarı mesajı
    ...
    # 6. Başarılı Oluşturmanın Ardından İndeksi Kaydetme (Yerel/Colab için)
    try:
        faiss.write_index(index, faiss_index_path)
        np.save(chunks_path, text_chunks)
        st.success("FAISS indexi ve metin parçaları diske kaydedildi.")
    except Exception as e:
        # Streamlit Cloud'da dosya yazma izni olmayabilir. Bu hata kritik değildir.
