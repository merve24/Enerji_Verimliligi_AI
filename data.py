...
def simple_query_streamlit(prompt, collection, api_key):
    ...
    try:
        ...
        # Stream'i tüket, her bir parçayı yield et ve akışı yumuşat
        if response_stream:
            for chunk in response_stream:
                
                # GÜVENLİK İYİLEŞTİRMESİ: chunk'ın kendisinin ve prompt_feedback'in varlığını kontrol et
                if chunk and chunk.text:
                    # YUMUŞAK AKIŞ EKLEME: Her karakter arasında bekleme süresi
                    for char in chunk.text:
                        yield char
                        time.sleep(0.015) 
                
                # GÜVENLİK İYİLEŞTİRMESİ: Eğer chunk boşsa, ama prompt_feedback alanı varsa (filtrelenme olabilir)
                ...
                # Ekstra Kontrol: Eğer chunk boşsa ve filtrelenme de yoksa, API'de sorun var demektir.
                elif chunk is None:
