...
def rag_query_streamlit(query, k=8): 
    ...
    # 💥 SON DÜZELTME: API yanıtı olan 'EmbedContentResponse' nesnesindeki vektöre 
    # Streamlit/Wrapper uyumluluğu için '.values' özelliği ile erişiyoruz. 
    # Bu, 'embedding' özelliği bulunamayan hataları (AttributeError) çözer.
    try:
        # Vektör verisini doğrudan çekme
        # response_embedding bir EmbedContentResponse nesnesidir. 
        # API'nin bu ortamdaki davranışına göre '.values' daha doğru olabilir.
        query_embedding = np.asarray(response_embedding.values, dtype='float32')
    except Exception as e:
        # Eğer bu temel erişim hala başarısız olursa, net bir hata mesajı ver.
