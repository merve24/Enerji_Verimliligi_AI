...
def rag_query_streamlit(query, k=8): 
    ...
    # 💥 KESİN VE TEK BİR DÜZELTME: API'den dönen yanıtın nesne yapısında (Pydantic/EmbeddingResponse) 
    # olduğunu ve FAISS'in beklediği 1536 boyutlu NumPy dizisine erişim için en güvenilir yolu deniyoruz.
    try:
        # En güncel ve tutarlı SDK erişim şekli: .embedding listesinden ilk elemanı al ve .values ile NumPy dizisine dönüştür.
        query_embedding = response_embedding.embedding[0].values
    except AttributeError:
        # Eğer '.values' yoksa (zaten NumPy array'iyse veya farklı bir yapıysa), sadece .embedding[0] deneriz.
        try:
             query_embedding = response_embedding.embedding[0]
        except AttributeError:
             # Eğer API eski bir sözlük yapısı döndürüyorsa (en az olası durum):
             query_embedding = response_embedding['embedding'][0]
