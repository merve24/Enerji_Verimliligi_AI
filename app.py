...
def rag_query_streamlit(query, k=8): 
    ...
    # nokta notasyonu ile erişimi zorluyoruz. (data.py ile tutarlı)
    try:
        # response.embedding, Embeddings nesnelerinin bir listesidir. 
        # İlk (ve tek) elemanın vektörünü alıyoruz.
        query_embedding = np.asarray(response_embedding.embedding[0], dtype='float32')
    except Exception as e:
        # Hata devam ederse, kullanıcıya net bir hata mesajı gösterip uygulamayı durduruyoruz.
        st.error(f"Vektörleştirme API yanıtı hatası: {e}. Lütfen API kütüphane versiyonunuzu kontrol edin.")
...
for msg in st.session_state.messages:
    ...
    avatar = "💡" if msg["role"] == "assistant"] else "👤" # Bu satırda hata varsa, düzeltilmesi gerekir.
    # Düzeltilmiş satır (Eğer hata varsa): avatar = "💡" if msg["role"] == "assistant" else "👤" 
