...
def rag_query_streamlit(query, k=8): 
    ...
    # 💥 KESİN DÜZELTME: API'den dönen EmbedContentResponse nesnesinden,
    # doğru vektör listesine erişim için nokta notasyonu kullanılır (.embedding). 
    # Önceki tüm hataları çözmek için karmaşık try/except bloğu kaldırıldı.
    # Listeden ilk (ve tek) vektör alınır.
