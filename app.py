...
def rag_query_streamlit(query, k=8): 
    ...
    # 💥 KESİN DÜZELTME: API'den dönen yanıtın bir sözlük gibi davranması veya 
    # FAISS'e uygun vektör dizisini içermesi gerektiği varsayımıyla, 
    # data.py'deki başarılı formata yakın bir sözlük erişimi kullanılıyor. 
    # Tek bir sorgu vektörünü döndüren en güvenilir yapı.
    try:
        # data.py'de başarılı olan sözlük erişimini deniyoruz.
        query_embedding = response_embedding['embedding'][0]
    except TypeError:
        # Eğer TypeError: 'EmbedContentResponse' object is not subscriptable hatası alınırsa (sözlük değilse), 
        # nesne olarak erişimi deniyoruz (ki bu da AttributeError vermişti).
        # Ancak bu son deneme, önceki hatayı veren satırdı. 
        # API'nin bu bağlamda bir liste döndürme ihtimali yüksek.
        # Bu nedenle, 'embedding' özelliğine erişip, ilk elemanı almayı tekrar deniyoruz.
        try:
             query_embedding = response_embedding.embedding[0]
        except AttributeError:
             # Eğer tüm bu yaklaşımlar başarısız olursa, API'nin en temel (raw) listesini çekmeyi deniyoruz.
