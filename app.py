...
def rag_query_streamlit(query, k=8): 
    ...
    # 💥 DÜZELTME: Güvenli erişim için, data.py'de çalışan sözlük yapısına zorluyoruz.
    try:
        # En eski ve uyumlu API formatı (sözlük) kullanılır.
        query_embedding = np.asarray(response_embedding['embedding'][0], dtype='float32')
    except Exception as e:
        # Hata devam ederse, kullanıcıya net bir hata mesajı gösterip devam etmesini engelliyoruz.
        st.error(f"Vektörleştirme API yanıtı hatası: {e}. Lütfen API kütüphane versiyonunuzu kontrol edin.")
        # Programın devam etmesini engellemek için hata fırlatılır.
