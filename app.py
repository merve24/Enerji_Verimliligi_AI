...
def rag_query_streamlit(query, k=8): 
    ...
    # 💥 SON DÜZELTME: API yanıtı olan 'EmbedContentResponse' nesnesinden vektörü 
    # doğrudan '.embedding' özelliği ile çekiyoruz. Bu, 'data.py' ile tutarlıdır ve 
    # önceki denemelerde kullanılan yanlış dizin erişimini ([0]) ortadan kaldırır.
    try:
        # Vektör verisini doğrudan çekme
        query_embedding = np.asarray(response_embedding.embedding, dtype='float32')
    except Exception as e:
        # Eğer bu temel erişim hala başarısız olursa, net bir hata mesajı ver.
        # Bu blok, 'gömme' özelliği olmadığı durumları yakalar (AttributeError).
