...
if prompt := st.chat_input("Enerji verimliliği hakkında bir soru sorun... 📝"):
    ...
    # 1. Aşama: simple_query_streamlit artık bir generator (akış) döndürür
    response_generator = simple_query_streamlit(prompt, collection, GEMINI_API_KEY)
    
    # 2. Aşama: Yanıtı karakter karakter ekrana yazdırmak için st.write_stream kullanılır
    # write_stream, generator'ı tüketir ve nihai tam metni (full_response) döndürür.
    full_response = st.chat_message("assistant", avatar="💡").write_stream(response_generator)
    
    # Yanıt geçmişe kaydedilir
    st.session_state.messages.append({"role": "assistant", "content": full_response})

    # Kaynak metni terminale yazdırma kısmı streaming nedeniyle kaldırıldı.
    # if source:
