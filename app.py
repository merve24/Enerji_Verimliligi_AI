...
def rag_query_streamlit(query, k=8): 
    ...
    # 💥 KESİN VE BASİT DÜZELTME: API'den dönen yanıtın 'embedding' özelliğine erişip
    # (bu bir liste olmalı) ilk (ve tek) elemanı alıyoruz. 
    # Ardından, olası tüm veri türü hatalarını gidermek için np.asarray ile
    # kesin olarak NumPy array'ine dönüştürüyoruz.
    try:
        # En güvenilir erişim yöntemi: .embedding[0] ve np.asarray ile dönüşüm
        query_embedding = np.asarray(response_embedding.embedding[0], dtype='float32')
    except Exception as e:
        # Son bir şans olarak, data.py'de çalışan sözlük erişimini deneriz
        try:
            query_embedding = np.asarray(response_embedding['embedding'][0], dtype='float32')
        except Exception:
            st.error(f"EmbedContent yanıtı hatalı formatta: {e}")
            # Hata devam ederse, geçici olarak sıfır vektör döndürülmeli veya uygulama durdurulmalı.
            # Ancak kullanıcı deneyimi için, bu noktada hata günlüğüne yazmak daha iyidir.
            raise ValueError("API'den gelen gömülü vektör verisine erişilemiyor. Lütfen API kütüphanenizi kontrol edin.")
